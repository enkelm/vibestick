...
- L3+R3 toggles mouse mode in any app context, including unknown apps. The
  ...
  first remains held and within an initial 250 ms chord window. Keep this
  timing in one place and adjust only with physical-use evidence.
...
- Mouse mode is session-only and starts off on every launch. It persists
  ...
  pause, or shutdown. A new connection or resumption never silently
  re-enters it. If Accessibility is unavailable, do not start emitting pointer
  or click events; report the missing permission rather than presenting an
  apparently working mouse mode.
...
the interruption; never carry a held drag into another app or surface.
...
Recognize the installed Delta macOS app by its verified bundle identity, not
by guessing a bundle ID from the display name or matching unrelated windows.
If its identity cannot be confirmed, retain the existing unknown-app safety
behavior. Delta gets a distinct app context and built-in preset; its operator
profile remains sparse, keyed by the app's bundle ID, so existing overrides
for that ID continue to win over the new preset. Unknown apps remain unbound
outside explicit mouse mode.
...
| B | Dismiss the current surface | Escape |
| X | Open Review Changes | Option-Shift-D |
| Y | Search commands and threads | Command-K |
...
| Left stick | Repeatable arrow navigation | Arrow keys |
| Right stick | Repeatable scrolling | Scroll wheel |

LT, RT, and R3 have no Delta base action. The system L3 and Share gestures
remain global; mouse mode replaces the base preset while selected. Preserve
the current profile/override and stick-mapping rules, and display the Delta
preset in the bindings overlay/editor with binding source information.
...
- chord recognition in both orders, deadline and release edges, duplicate
  input, incomplete chords falling back exactly once, and no L3 wheel or
  Slack R3 mute action on a completed chord;
...
- mouse down/up and drag behavior, with button release and zero stale output
  at every interruption listed above;
- mode persistence across app changes, exit on pause/disconnect/shutdown,
  permission-denied behavior, and menu recovery;
- Delta identification, shortcut table, sparse-override precedence, unknown
  app safety, and no default consequential Delta action.
...
denied/granted. Check that the actual Delta install is positively identified
and its mapped shortcuts do what their labels say. Bugs found during use are
tracked and fixed as they arise; this list is not the earlier milestone's
